...
func TestDeleteExpiredMCPTokens(t *testing.T) {
	db := testDB(t)
	u, err := CreateUser(context.Background(), db, "tokuser", "", "pw123456", "", false, false, sql.NullInt64{})
	if err != nil {
		t.Fatalf("create user: %v", err)
	}

	// A live token (expires 90 days out).
	_, live, err := CreateMCPToken(context.Background(), db, u.ID, "", "live")
	if err != nil {
		t.Fatalf("create live token: %v", err)
	}
	// A token that expired 40 days ago (past the grace cutoff).
	_, expOld, _ := CreateMCPToken(context.Background(), db, u.ID, "", "expired-old")
	...
	_, expRecent, _ := CreateMCPToken(context.Background(), db, u.ID, "", "expired-recent")
	if _, err := db.Exec(`UPDATE mcp_tokens SET expires_at = ? WHERE id = ?`,
	...
	n, err := DeleteExpiredMCPTokens(context.Background(), db, cutoff)
	if err != nil {
	...
	if _, err := ValidateMCPTokenByID(context.Background(), db, live.ID); err != nil {
		t.Errorf("live token should survive: %v", err)
	}
}

// ValidateMCPTokenByID is a tiny test-only existence probe (avoids re-hashing).
func ValidateMCPTokenByID(ctx context.Context, db *sql.DB, id int64) (int64, error) {
	var uid int64
...
func TestDeleteOrphanDCRClients(t *testing.T) {
	db := testDB(t)
	u, err := CreateUser(context.Background(), db, "dcruser", "", "pw123456", "", false, false, sql.NullInt64{})
	if err != nil {
		t.Fatalf("create user: %v", err)
	}
	...
	orphan, _, err := RegisterDCRClient(context.Background(), db, "orphan", []string{"https://claude.ai/cb"}, "client_secret_post")
	if err != nil {
	...
	active, _, err := RegisterDCRClient(context.Background(), db, "active", []string{"https://claude.ai/cb"}, "client_secret_post")
	if err != nil {
		t.Fatalf("register active: %v", err)
	}
	backdate(active.ClientID)
	if _, _, err := CreateMCPToken(context.Background(), db, u.ID, active.ClientID, "tok"); err != nil {
		t.Fatalf("create token for active client: %v", err)
	}

	// Fresh: orphan but registered just now → within grace, must be retained.
	fresh, _, err := RegisterDCRClient(context.Background(), db, "fresh", []string{"https://claude.ai/cb"}, "client_secret_post")
	if err != nil {
		t.Fatalf("register fresh: %v", err)
	}

	n, err := DeleteOrphanDCRClients(context.Background(), db, time.Now().Add(-30*24*time.Hour))
	if err != nil {
	...
	if _, err := GetDCRClient(context.Background(), db, orphan.ClientID); err != ErrNotFound {
		t.Errorf("orphan should be deleted, got err=%v", err)
	}
	if _, err := GetDCRClient(context.Background(), db, active.ClientID); err != nil {
		t.Errorf("active client should survive: %v", err)
	}
	if _, err := GetDCRClient(context.Background(), db, fresh.ClientID); err != nil {
