...
func TestAthletes_EditForm_CoachCanView(t *testing.T) {
	db := testDB(t)
	tc := testTemplateCache(t)
	coach := seedCoach(t, db)
	athlete := seedAthlete(t, db, "Alice", "foundational")
	...
	h := &Athletes{DB: db, Templates: tc}
	req := requestWithUser("GET", "/athletes/"+itoa(athlete.ID)+"/edit", nil, coach)
	req.SetPathValue("id", itoa(athlete.ID))
	rr := httptest.NewRecorder()
	h.EditForm(rr, req)
...
func TestAthletes_EditForm_NonCoachForbidden(t *testing.T) {
	db := testDB(t)
	tc := testTemplateCache(t)
	athlete := seedAthlete(t, db, "Kid", "")
	nonCoach := seedNonCoach(t, db, athlete.ID)
	...
	h := &Athletes{DB: db, Templates: tc}
	req := requestWithUser("GET", "/athletes/"+itoa(athlete.ID)+"/edit", nil, nonCoach)
	req.SetPathValue("id", itoa(athlete.ID))
	rr := httptest.NewRecorder()
	h.EditForm(rr, req)
...
func TestAthletes_EditForm_NotFound(t *testing.T) {
	db := testDB(t)
	tc := testTemplateCache(t)
	coach := seedCoach(t, db)

	h := &Athletes{DB: db, Templates: tc}
	req := requestWithUser("GET", "/athletes/999/edit", nil, coach)
	req.SetPathValue("id", "999")
	rr := httptest.NewRecorder()
	h.EditForm(rr, req)
...
func TestAthletes_Update_NonCoachForbidden(t *testing.T) {
	db := testDB(t)
	tc := testTemplateCache(t)
	athlete := seedAthlete(t, db, "Kid", "")
	nonCoach := seedNonCoach(t, db, athlete.ID)
	...
	req := requestWithUser("POST", "/athletes/"+itoa(athlete.ID), form, nonCoach)
	req.SetPathValue("id", itoa(athlete.ID))
	rr := httptest.NewRecorder()
	h.Update(rr, req)
...
func TestAthletes_Update_EmptyName(t *testing.T) {
	db := testDB(t)
	tc := testTemplateCache(t)
	coach := seedCoach(t, db)
