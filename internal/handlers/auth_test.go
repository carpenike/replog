...
func TestAuth_LoginPage_RedirectsWhenLoggedIn(t *testing.T) {
	...
	user, err := models.CreateUser(db, "coach", "password123", "", true, sql.NullInt64{})
	if err != nil {
...
func TestAuth_LoginSubmit_Success(t *testing.T) {
	...
	_, err := models.CreateUser(db, "coach", "password123", "c@test.com", true, sql.NullInt64{})
	if err != nil {
		t.Fatalf("create user: %v", err)
	}
...
func TestAuth_LoginSubmit_InvalidCredentials(t *testing.T) {
	...
	_, err := models.CreateUser(db, "coach", "password123", "", true, sql.NullInt64{})
	if err != nil {
		t.Fatalf("create user: %v", err)
	}
